...
data class DefaultRegisterMeetingOfferRequest(
    @Schema(description = "워크스페이스(그룹) 고유 키") override val workspaceId: Int,
    @Schema(description = "공고 고유 키") override val openingId: Int,
    @Schema(description = "지원자 고유 키 모음") override val applicantsIds: Set<Int> = emptySet(),
    @Schema(description = "면접관(유저) 고유 키 모음") override val recruiterIds: Set<Int> = emptySet(),
...
) : AbstractMeetingOfferRequest(workspaceId, openingId, applicantsIds, recruiterIds, time, content, attachment)